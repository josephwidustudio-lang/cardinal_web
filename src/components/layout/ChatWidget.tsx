...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: '¡Hola! Soy el asesor virtual de Cardinal. ¿En qué puedo ayudarte?' }
  ])
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.message || 'Lo siento, ocurrió un error. Intentá de nuevo.' }])
    } catch {
    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          {/* Header */}
          <div style={{
            padding: '1rem 1.2rem', background: '#0A2D38',
            ...
            <button onClick={() => setOpen(false)} style={{
              background: 'transparent', border: 'none', color: '#7A9BA8',
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                <div style={{
                  padding: '0.7rem 1rem', background: 'rgba(206,162,121,0.08)',
                  border: '1px solid rgba(206,162,121,0.15)', color: '#CEA279', fontSize: '0.9rem',
                  letterSpacing: '0.2em',
                }}>···</div>
              </div>
            )}
          ...
          {/* Input */}
          <div style={{
            padding: '0.8rem', borderTop: '1px solid rgba(206,162,121,0.15)',
      ...
      <button onClick={() => setOpen(!open)} style={{
        position: 'fixed', bottom: '2rem', left: '2rem', zIndex: 300,
